import { ComplaintLogModal } from "@/components/ComplaintLogModal";
...
export const ComplaintsLog: React.FC<ComplaintsLogProps> = () => {
  ...
  return (
    <div className="flex flex-col w-full h-full">
      ...
      <Header
        icon={<ComplaintsIcon />}
        title={tab === "Pending" ? "Complaints: Log" : "Log Complaint"}
      />
      ...
      <div className="px-5 pt-2 pb-8 flex-1 overflow-y-auto">
        <div className="max-w-[95.625rem] w-full h-full mx-auto flex flex-col space-y-4">
          ...
          <div className="flex justify-between flex-wrap gap-4 items-center">
            <div className="flex min-w-[12rem] h-[40px] border border-[#D0D5DD] rounded-[8px] shadow-[0px_1px_2px_0px_#1018280D] overflow-hidden">
              {["Pending", "Resolved"].map((tabName, index) => (
                <button
                  key={index}
            ...
            <button
              className="max-w-[8.5rem] w-full h-[36px] bg-[#014DAF] hover:bg-[#014DAF]/80 text-white cursor-pointer px-3 py-2 rounded-[4px] text-xs flex items-center gap-2 font-medium"
              onClick={() => setModalOpen(true)}
            >
              <FilePlusIcon /> <span>Log Complaint</span>
            </button>
          </div>
                ...
                <tbody>
                  {complaints.map((row: Complaint, idx: number) => (
                    <tr
                      ...
                      onClick={() => navigate(`/complaint-details/${row.id}`)}
                    >
      ...
      <ComplaintLogModal open={modalOpen} onClose={() => setModalOpen(false)} />
    </div>
  );
};
